<?php
// $Id: left.php,v 1.6 2008/04/06 23:08:27 agentken Exp $

/**
 * @file
 * A two-column page layout file for MySite with one narrow rail.  Optional.
 *
 ...
 */
function theme_mysite_left_layout($content) {
  // break the array into pieces
  $owner = $content['owner'];
  $mysite = $content['mysite'];
  $columns = mysite_layout_left();
  ...
  foreach ($data as $col => $set) {
    if ($col == 0) {
      $output .= '<div class="mysite-sortable mysite-single-width" id="mysite-sort'. $col .'">';
    }
    else {
 ...
 */
function mysite_layout_left() {
  $data = array();
  $data['regions'] = array('0' => t('Sidebar'), '1' => t('Main'));
  $data['count'] = count($data['regions']);
  $data['name'] = t('Left sidebar');
  $data['description'] = t('A large main area with left sidebar.');
  $data['image'] = 'left.png';
  return $data;
}
