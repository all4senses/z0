...
echo '<div class="title2">Описания и контакты:</div><div class="title">Производители и поставщики</div>';
...
$tids_search = taxonomy_get_term_by_name('Производители / поставщики');
...
if($_GET['sort'] == 'alfa')
{
	...
	echo '<div class="sort_link">Сортировка:  <a class="active" href="' . url('node/' . arg(1)) . '?sort=alfa">по странам и алфавиту</a>' . ' / ' . '<a href="' . url('node/' . arg(1)) . '?sort=type">по типу деятельности и странам</a></div>';
	
	cache_clear_all('*', 'cache_views', true);
	$view = views_get_view('mc_descr_all');
...
if(!$_GET['sort'] OR $_GET['sort'] == 'type')
{
	...
	//echo 'Сортировка:' . l(' по дате побликации', url('node/' . arg(1)) . '?sort=date') . '; ' . l('По типу товаров', url('node/' . arg(1)) . '?sort=type') . '<br>';
	echo '<div class="sort_link">Сортировка:  <a href="' . url('node/' . arg(1)) . '?sort=alfa">по странам и алфавиту</a>' . ' / ' . '<a class="active" href="' . url('node/' . arg(1)) . '?sort=type">по типу деятельности и странам</a></div>';
