...
function _phptemplate_variables($hook, $vars) {
    ...
    if ($hook == 'node') 
    {
        ...
        // formiruem peremennye dlia verhney shapki s menu
        if ($vars['node']->type == 'zp_footer') 
        {
            zp_functions_getDefaultWelcomeAndExplain($vars, arg(0), arg(1));
            ...
            zp_functions_setFirstMenu_andSession($vars);

            // make user_menu variable
            zp_functions_setUserMenu_andSession($user, $vars, 'logged out');
            //zp_functions_setUserMenu_andSession($user, $vars);
...
//--------------------------------------------------------------------------------------------
            ...
            zp_functions_setSecondMenu_andSession(arg(0), arg(1), null, $vars, $cur_shop_info, $argument_shop_tid, $argument_rajon_tid, $argument_city_tid, $argument_country_tid);
            /*
            ...
            */
            ...
            zp_functions_setCitiesMenu_andSession($vars);
            ...
            zp_functions_setCurrentPlace_defaultSecondMenu_andSession($vars);
            ...
            zp_function_setHeaderPic($vars, arg(1), false);
            
            // send to js current place tid
            zp_functions_sendCurrentPlaceTid_toJs();
            
        } // end of if($vars['node']->type == 'zp_footer')
        // конец формирования переменных для шапки сайта
        
        
        
        // make variable breadcrumb for catalog
        //if ($vars['node']->type == 'c_breadcrumb' AND $vars['node']->title == 'c_breadcrumb_01') 
        if ($vars['node']->type == 'zp_header') 
        {
            zp_functions_setBreadCrumb_andSession($vars, arg(0), arg(1));
        }

        // make side menu block variable 
        zp_functions_setSideMenu_andSession($vars);
        ...
        if ($vars['page']) 
        {
            ...
            {
                case 'product_set_1':
                      zp_functions_prepareSomeData($vars, $user->uid);
                case 'product_set_2':
                case 'c_department':
                case 'c_shop':
                case 'c_rajon':
                ...
                case 'c_country':
                    ...
                    // pass to js current place
                    drupal_add_js(    
                    "
                        zp_type = 'place';
                        zp_nid = " . $vars['node']->nid . ";
                        zp_node_type = '" . $vars['node']->type . "';
               
                    ", 'inline');

                    // image_src tag for search and social were set in templates
                    break;
            ...
            //if($user->uid == 1)
            {
                drupal_add_js('sites/all/modules/_Jstools/jquery.cookie.js');
                drupal_add_js('sites/all/modules/_ZP_modules/_JS/current_place.js');
            }
...
//pravim shablon s panelyami - ubiraem zagolovki po umolchaniyu i t.d.
/**
...
function phptemplate_comment($comment, $links = 0) {
    return _phptemplate_callback('comment', array(
                ...
                'submitted' => t('Submitted by !a on @b.', array('!a' => theme('username', $comment),
